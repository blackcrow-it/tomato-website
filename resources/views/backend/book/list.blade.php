...
<div class="row mb-2">
    <div class="col-sm-6">
        <h1 class="m-0 text-dark">Sách</h1>
    </div><!-- /.col -->
    <div class="col-sm-6">
        <div class="float-sm-right">
            <a href="{{ route('admin.book.add') }}" class="btn btn-primary"><i class="fas fa-plus-circle"></i> Thêm mới</a>
        </div>
    </div><!-- /.col -->
</div>
            ...
            @foreach($list as $item)
                <tr>
                    ...
                    <td class="text-nowrap">
                        <form action="{{ route('admin.book.delete', [ 'id' => $item->id ]) }}" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa sách này?')">
                            @csrf
                            <a href="{{ route('admin.book.edit', [ 'id' => $item->id ]) }}" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i> Sửa</a>
...
<script>
    ...
    $('.js-order-in-category').change(function () {
        ...
        $.post('{{ route("admin.book.order_in_category") }}', {
            id: $(that).data('id'),
    ...
    $('.js-order-in-position').change(function () {
        ...
        $.post('{{ route("admin.book.order_in_position") }}', {
            id: $(that).data('id'),
